            ...
            @if ( !empty(request('sale')) || !empty(request('location')) || !empty(request('type')) || !empty(request('price')) || !empty(request('bedrooms')) )
                <h3 class="text-sm mt-1 font-normal">
                    ...
                    @if (request('price') == 100000)
                        within 0 {{ __('DT') }} - 1,00,000 {{ __('DT') }} Price Range
                    @elseif (request('price') == 200000)
                        within 1,00,000 {{ __('DT') }} - 2,00,000 {{ __('DT') }} Price Range
                    @elseif (request('price') == 300000)
                        within 2,00,000 {{ __('DT') }} - 3,00,000 {{ __('DT') }} Price Range
                    @elseif (request('price') == 400000)
                        within 3,00,000 {{ __('DT') }} - 4,00,000 {{ __('DT') }} Price Range
                    @elseif (request('price') == 500000)
                        and More than 5,00,000 {{ __('DT') }} Price
                    @endif
                    from
                ...
                <h4 class="text-sm font-normal"><span class="font-bold">{{ $latest_properties->total() }}</span> Properties Found</h4>
            @endif
